...
module.exports = {
	...
	execute: async ({ interaction, client }) => {
		...
		const modalPopup = new ModalBuilder()
			.setCustomId(`report-${interaction.user.id}-${client.user.id}`).setTitle(client.user.username + '\'s Bug Report!');
		...
		const res = interaction.awaitModalSubmit({ filter, time: 150_000 })
			.then(async (modal) => {
				...
				modal.followUp({ content: `Thank you for helping us make ${client.user.username} even better.`, ephemeral: true });
				return true;
